...
SafeAreaView,StatusBar,KeyboardAvoidingView,TouchableWithoutFeedback,
TouchableOpacity,Button,ImageBackground,Platform,BackHandler
} from 'react-native';
...
onYourSendEmailFunction = async() =>{
  ...
  if (this.state.email === ""){
    ...
    const sendRequest = sendGridEmail(SENDGRIDAPIKEY, this.state.email, FROMEMAIL, SUBJECT, ContactDetails )
...
testInputCode = async() =>{
    ...
    else if(this.state.emailcode !== save_random_code || this.state.emailcode===""){
...
onChangePassword = async() =>{
    ...
    if(this.state.password ==="" || this.state.repeat_password === "" || this.state.emailcode ==="")
    {
      ...
      usersRef.orderByChild('email').equalTo(this.state.email).on('child_added', function(snapshot){
  ...
  render() {
    return (
      <SafeAreaView style={styles.AndroidSafeArea} >
        <StatusBar barStyle = "light-content" hidden = {false}  backgroundColor="#C2185B" translucent = {false} />
        <KeyboardAvoidingView behavior="padding" style={styles.container} >
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <ImageBackground
              style={styles.container}
              source={{ uri: 'https://raw.githubusercontent.com/flatlogic/react-native-starter/master/assets/images/background.png' }}>
              <View style={styles.container}>

                <Text style={styles.textForgot}>Forgot Password ?</Text>
           
                <View style={styles.groupInput}>
                  <Text style={styles.textStep}>Step 1: Please enter your email and we'll send you the code to reset your password</Text>
                  <TextInput
                  ...
                  />
                  <TouchableOpacity style={styles.btnSendMail} onPress={() => this.onYourSendEmailFunction()}>
                ...
                <View style={styles.groupInput}>
                  <Text style={styles.textStep}>Step 2: Check your email and input your code</Text>
                  <TextInput
                    style={styles.textofInput}
                    underlineColorAndroid='white'
                    autoCapitalize="none"
                  ...
                  />
                  <TouchableOpacity style={styles.btnSendMail} onPress={() => this.onChangePassword()}>
